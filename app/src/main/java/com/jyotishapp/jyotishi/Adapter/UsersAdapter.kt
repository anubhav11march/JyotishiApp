...
class UsersAdapter(
...
): RecyclerView.Adapter<UsersAdapter.RemoteViewHolderTest>() {
    ...
    override fun onBindViewHolder(holder: RemoteViewHolderTest, position: Int) {
        ...
        mRtcEngine?.muteRemoteVideoStream(asyncListDiffer.currentList[position], false)

        val surface = RtcEngine.CreateRendererView(holder.view.context)

        surface.tag = asyncListDiffer.currentList[position]

        mRtcEngine!!.setupRemoteVideo(VideoCanvas(surface, VideoCanvas.RENDER_MODE_HIDDEN, asyncListDiffer.currentList[position]))
        ...
        FirebaseDatabase.getInstance().getReference("StreamMetaData")
                .child("${asyncListDiffer.currentList[position]}")
